		...
		teu.getBtnSalvar().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (validarDados(login, senha, email)) {
					...
					Fachada.getInstance().updateUsuario(login, teu.getUsuario().getEndereco());
					if (teu.getUsuario().getTipo()=="Cliente") {
						Fachada.getInstance().updateCliente(login, teu.getUsuario().getLogin());
					}
					if (teu.getUsuario().getTipo()=="Fornecedor") {
						Fachada.getInstance().updateFornecedor(login, teu.getUsuario().getLogin());
					}
